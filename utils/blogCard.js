...
const blogCard = async (data, hostname, large = false, theme = "light") => {
  ...
  return `<svg  ${large ? ` width="302" height="522"` : ` width="174" height="300" `} viewBox="0 0 302 522" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    <text fill="${color.author_name}"  font-family="Helvetica" font-size="14" letter-spacing="0em" x="71" y="38"><tspan font-weight="bold">${author.name}</tspan>
    <tspan fill="${color.author_username}" fill-opacity="0.75"  font-family="Helvetica" font-size="14" ${large ? "" : `font-weight="bold"`} letter-spacing="0em">@${author.username}</tspan></text>
    <text fill="${color.pub_date}" fill-opacity="0.75"  font-family="Helvetica" font-size="12" ${large ? "" : `font-weight="bold"`} letter-spacing="0em"><tspan x="71" y="55">${blogDate}</tspan></text>
    <foreignObject
          x="16" y="80" width="260" height="120">
          <p
            xmlns="http://www.w3.org/1999/xhtml"
            xlink:href="#blogName"
            style="color:${color.blog_title};font-family:Helvetica;font-size:20px;font-weight:bold;margin:0;padding:0;font-weight=bold"
            letter-spacing="0em"
            >${title}
            </p>
    </foreignObject >
    ...
    <foreignObject
          ...
          <p
            xmlns="http://www.w3.org/1999/xhtml"
            xlink:href="#blogName"
            style="color:${color.blog_brief};font-family:Helvetica;font-size:14px;margin:0;padding:0;font-weight=bold"
            letter-spacing="0em"
            >${brief}
            </p>
    </foreignObject >
