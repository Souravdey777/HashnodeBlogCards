...
const blogCard = async (data, hostname, large = false, dark = false) => {
  ...
  return `<svg  ${large ? ` width="302" height="522"` : ` width="174" height="300" `} viewBox="0 0 302 522" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    <rect x="1" y="1" width="300" height="520" rx="4" ${dark ? `fill="#232626" stroke="#AEAEAE"` : `fill="white" stroke="#E5E7EB"`} stroke-width="1"/>
    ...
    <text fill="${dark ? `#F5F5F5` : `#424242`}"  font-family="Helvetica" font-size="14" letter-spacing="0em" x="71" y="38"><tspan font-weight="bold">${author.name}</tspan>
    <tspan fill="${dark ? `#AEAEAE` : `#424242`}" fill-opacity="0.75"  font-family="Helvetica" font-size="14" ${large ? "" : `font-weight="bold"`} letter-spacing="0em">@${author.username}</tspan></text>
    <text fill="${dark ? `#AEAEAE` : `#424242`}" fill-opacity="0.75"  font-family="Helvetica" font-size="12" ${large ? "" : `font-weight="bold"`} letter-spacing="0em"><tspan x="71" y="55">${blogDate}</tspan></text>
    <foreignObject
          x="16" y="80" width="260" height="120">
          <p
            xmlns="http://www.w3.org/1999/xhtml"
            xlink:href="#blogName"
            style="color:${dark ? `#F5F5F5` : `#333333`};font-family:Helvetica;font-size:20px;font-weight:bold;margin:0;padding:0;font-weight=bold"
            letter-spacing="0em"
            >${title}
            </p>
    </foreignObject>
    <text fill="${dark ? `#AEAEAE` : `#616161`}"  font-family="Helvetica" font-size="14" font-weight="bold" letter-spacing="0em"><tspan x="16" y="215">${hostname}</tspan></text>
    <foreignObject
          x="16" y=${coverImage !== "" ? `"370"` : `"250"`} width="260" height="120">
          <p
            xmlns="http://www.w3.org/1999/xhtml"
            xlink:href="#blogName"
            style="color:${dark ? `#AEAEAE` : `#333333`};font-family:Helvetica;font-size:14px;margin:0;padding:0;font-weight=bold"
            letter-spacing="0em"
            >${brief}
            </p>
    </foreignObject>
