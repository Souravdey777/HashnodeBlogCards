...
function GetHashnodeBlogBySequence(props) {
    ...
    const getData = () => {
        console.log(API_URL)
        if (`https://hashnode-blog-cards.souravdey777.vercel.app/api/getHashnodeBlogBySequence?username=${params.username}&sequence=${params.sequence}&large=${params.large}&theme=${params.theme}` !== API_URL) {
            setisLoading(true)
            console.log("SEND clicked");
            setAPI_URL(`https://hashnode-blog-cards.souravdey777.vercel.app/api/getHashnodeBlogBySequence?username=${params.username}&sequence=${params.sequence}&large=${params.large}&theme=${params.theme}`);
            console.log(API_URL);
        }
    }
    ...
    const handlesequence = (event) => {
        setparams({ ...params, sequence: event.target.value });
        console.log({ ...params, sequence: event.target.value })
    }
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ display: "flex", flexDirection: "row" }}>
                    ...
                    <input
                        ...
                        type="number" min="1" value={params.sequence} onChange={handlesequence} />
                </div>
...
export default GetHashnodeBlogBySequence;