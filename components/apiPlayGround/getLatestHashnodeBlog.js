...
function GetLatestHashnodeBlog(props) {
    ...
    const getData = () => {
        console.log(API_URL)
        if (`https://hashnode-blog-cards.souravdey777.vercel.app/api/getLatestHashnodeBlog?username=${params.username}&limit=${params.limit}&large=${params.large}&theme=${params.theme}` !== API_URL) {
            setisLoading(true)
            console.log("SEND clicked");
            setAPI_URL(`https://hashnode-blog-cards.souravdey777.vercel.app/api/getLatestHashnodeBlog?username=${params.username}&limit=${params.limit}&large=${params.large}&theme=${params.theme}`);
            console.log(API_URL);
        }
    }
    ...
    return (
        <div className={styles.PlayGround}>
            <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ display: "flex", flexDirection: "row" }}>
                    <div className={styles.labels}>Enter the limit</div>
                    <input
                        className={styles.textBox}
                        placeholder="Enter the limit..."
                        type="number" min="1" value={params.limit} onChange={handlelimit} />
                </div>
